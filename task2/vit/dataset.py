...
class CIFAR_100_Dataset(Dataset):
    def __init__(self, data, label, augment=None, transforms=None, **kwargs):
    ...
    def _transform_img(self, img):
        ...
        return self.transforms(normalized_img.permute(2, 0, 1)).permute(1, 2, 0)
    
    def __getitem__(self, idx):
        if isinstance(idx, int):
            prob = np.random.uniform()
            img = self._transform_img(self.data[idx])
    ...
    def _mixup_aug(self, img, label):
        ...
        aug_img = self._transform_img(self.data[aug_idx])
